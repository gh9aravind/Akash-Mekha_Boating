...
**Easier option:** Step 10 below sets up `upload.html` — a page where Akash/Subhash can upload hero, gallery, and host photos straight from their phone gallery, no GitHub needed. Use that once it's set up. The steps below are the manual/GitHub route, still needed for the cover image either way.
...
## 10. Photo Upload Portal (`upload.html`) — no more GitHub for photos

A PIN-protected page where Akash/Subhash can replace any site photo straight from their phone gallery. Uploads go to **your Google Drive** (via Apps Script — no external image host or API key needed) and the main site picks them up automatically.

**How it fits together:**
- `upload.html` → uploads a photo → `Code.gs` saves it to a Drive folder called **"Malarikkal Website Images"** and records the link in a new **GalleryImages** sheet tab.
- `index.html` now fetches that sheet on every page load and uses those photos automatically — falling back to the local `images/*.jpg` files (and then the gradient placeholder) if a slot hasn't been uploaded yet. Nothing breaks either way.
- **Exception:** the WhatsApp/link-preview cover image (`og:image`) still must go through GitHub as `images/cover.jpg` — link-preview crawlers don't run JavaScript, so they can't see a dynamically-fetched photo. Everything else (hero, gallery, host photos, developer photo) works through the new portal.

**Setup — reuses the same `DASHBOARD_ACCESS_KEY` from Step 9, nothing new to generate:**

1. In `upload.html`, set the same 3 values as `admin.html`:
   ```js
   const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycb.../exec";
   const DASHBOARD_ACCESS_KEY = "your-long-random-string-here"; // same as Code.gs and admin.html
   const DASHBOARD_PIN = "1234"; // can be the same PIN as admin.html, or different
   ```
2. Upload `upload.html` to the same GitHub repo, same folder as `index.html` and `admin.html`.
3. Optional: run `testImageUpload` once in the Apps Script editor to confirm Drive + the GalleryImages sheet are wired correctly (uploads a tiny placeholder to the "hero" slot — just re-upload a real photo over it afterward from the portal).
4. Open `https://gh9aravind.github.io/Akash-Mekha_Boating/upload.html`, log in with the PIN (shared session with `admin.html` — unlocking one unlocks both on that device), and start replacing photos.

**A few practical notes:**
- Photos are automatically resized/compressed in the browser before upload (max 1600px, ~80% JPEG quality) — keeps uploads fast on mobile data and Drive storage small.
- Re-uploading a slot replaces it — the old Drive file is trashed automatically so storage doesn't pile up.
- The first time `DriveApp` runs, Google may ask you to re-authorize the script (same "unsafe app" warning as before — normal for your own script).
